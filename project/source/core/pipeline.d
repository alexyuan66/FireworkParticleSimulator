...
class Pipeline{
    ...
    GLuint CompilePipeline(string pipelineName, string vertexShaderSourceFilename, string fragmentShaderSourceFilename){
        ...
        const GLint lenvshader = cast(const int)vertexSource.length;
        const GLint lenfshader = cast(const int)fragmentSource.length; 

        // Compile vertex shader
        vertexShader = glCreateShader(GL_VERTEX_SHADER);
        const char* vSource = vertexSource.ptr;
        glShaderSource(vertexShader, 1, &vSource, &lenvshader);
        ...
        glShaderSource(fragmentShader, 1, &fSource, &lenfshader);
