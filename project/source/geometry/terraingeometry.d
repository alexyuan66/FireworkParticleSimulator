...
class SurfaceTerrain: ISurface{
    ...
    override void Render(){
        ...
        {
            GLuint startIndex = z * (2 * mXDimensions);
            GLuint numIndices = 2 * mXDimensions;
            glDrawElements(GL_TRIANGLE_STRIP, numIndices, GL_UNSIGNED_INT, cast(void*)(startIndex * GLuint.sizeof));
        }
    ...
    void MakeTerrain(uint xDim, uint zDim, string heightmap_file){
        ...
        for(int z=0; z < zDim; z++){
            for(int x=0; x < xDim; x++){
                ...
                // TODO
                float height = cast(float)height_values[z * xDim * 3 + x * 3]; // RGB
                float u = cast(float)x;
                float v = cast(float)z;

                mVertices ~= VertexFormat3F2F([x * 1.0f, height, z * 1.0f], [u, v]);
            }
        }
