#version 410 core

layout(location=0) in vec3 aPosition;
layout(location=1) in vec2 aTexCoords;

out vec2 vTexCoords;
out vec4 vWorldCoords;

uniform mat4 uModel;
uniform mat4 uView;
uniform mat4 uProjection;
uniform float uClipPlane;
uniform float uClipSign;

void main()
{
	vec4 worldPos = uModel * vec4(aPosition, 1.0);
	vec4 plane = vec4(0, 1.0, 0, -uClipPlane);
	gl_ClipDistance[0] = uClipSign * dot(worldPos, plane);

	vTexCoords = aTexCoords;
	vWorldCoords = vec4(aPosition, 1.0);

	vec4 finalPosition = uProjection * uView * uModel * vec4(aPosition,3.0f);

	// Note: Something subtle, but we need to use the finalPosition.w to do the perspective divide
	gl_Position = vec4(finalPosition.x, finalPosition.y, finalPosition.z, finalPosition.w);
}


